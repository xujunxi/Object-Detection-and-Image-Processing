...
class YOLOLoss(nn.Module):
    ...
    def get_target(self, target, anchors, in_w, in_h, ignore_threshold):
        ...
        for b in range(bs):
            ...
            #Calc the Feature Center in the Feature Map
            gis = torch.floor(gxs)
            gjs = torch.floor(gys)
            ...
            #Calc the Anchor with most overlapping areas
            best_ns = torch.argmax(anch_ious,dim=-1)
            for i, best_n in enumerate(best_ns):
                if best_n not in anchor_index:
                    continue
                #Get the coordinate of the Anchor
                gi = gis[i].long()
                gj = gjs[i].long()
                gx = gxs[i]
                gy = gys[i]
    ...
    def get_ignore(self,prediction,target,scaled_anchors,in_w, in_h,noobj_mask):
        ...
        #Anchor Center Point adjustment parameters
        ...
        #Get Anchor with Center Point at the corner of pixel
        ...
        #Calc adjusted center and width and height
        ...
        for i in range(bs):
            ...
            #Calc True Box in the feature layer's scale
            if len(target[i]) > 0:
